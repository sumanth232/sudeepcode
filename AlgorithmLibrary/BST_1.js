...
{
	//this.insertField = addControlToAlgorithmBar("Text", "click me");
	//this.insertField.onkeydown = this.returnSubmit(this.insertField,  this.insertCallback.bind(this), 3);
	...
	//this.HeightButton = addControlToAlgorithmBar("Button", "Height");
	//this.HeightButton.onclick = this.HeightCallback.bind(this);
...
{
	//totalNodes = Math.floor(Math.random() * 17) + 1;
	//for( i = 0; i < totalNodes; i++){     
		this.implementAction(this.insertElement.bind(this), Math.floor(Math.random() * 99) + 1);
	//}
}
...
BST.prototype.HeightCallback = function(event)
{
	this.implementAction(this.Height.bind(this),"");						
}

BST.prototype.Height = function(tree)
{
 ...
 if( this.treeRoot != null)
     {
		...
		this.xPosOfNextLabel = BST.FIRST_HEIGHT_POS_X;
		this.yPosOfNextLabel = this.first_HEIGHT_pos_y;
		this.HEIGHTTreeRec(this.treeRoot);

	}
	return this.commands;
}

BST.prototype.HEIGHTTreeRec = function(tree)
...
BST.prototype.printTree = function(unused)
{
	...
	if (this.treeRoot != null)
	{
		...
		/*this.cmd1("CreateHighlightCircle", this.highlightID, BST.HIGHLIGHT_CIRCLE_COLOR, this.treeRoot.x, this.treeRoot.y);*/
		this.xPosOfNextLabel = BST.FIRST_PRINT_POS_X;
		this.yPosOfNextLabel = this.first_print_pos_y;
		this.printTreeRec(this.treeRoot);
		this.cmd1("Delete",  this.highlightID);
		this.cmd1("Step")
...
BST.prototype.printTreeRec = function(tree)
{
	...
	if (tree.left != null)
	{
		/*this.cmd1("Move", this.highlightID, tree.left.x, tree.left.y);*/
		this.printTreeRec(tree.left);
	//this.cmd1("Move", this.highlightID, tree.x, tree.y);				
		this.cmd1("Step");
	}
	var nextLabelID = this.nextIndex++;
	this.cmd1("CreateLabel", nextLabelID, tree.data, tree.x, tree.y);
	this.cmd1("SetForegroundColor", nextLabelID, BST.PRINT_COLOR);
	this.cmd1("Move", nextLabelID, this.xPosOfNextLabel, this.yPosOfNextLabel);
	this.cmd1("Step");
	...
	if (tree.right != null)
	{
/*		this.cmd1("Move", this.highlightID, tree.right.x, tree.right.y);*/
		this.printTreeRec(tree.right);
//		this.cmd1("Move", this.highlightID, tree.x, tree.y);	
		this.cmd1("Step");
	}
	return;
//window.alert(command1.join('|'));
}
...
{	
	...
	//this.HeightButton.disabled = true;
}
...
{	
	...
	//this.HeightButton.disabled = false;
}
